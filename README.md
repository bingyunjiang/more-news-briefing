...
`more-news-briefing` 与常见日报工具的差异：聚合、摘要、多视角、专题监测、去重已是行业普遍能力，本 skill 的差异不在“多一个摘要功能”，而在把新闻检索之后的编辑判断、证据边界、认知延伸、验收阻断和跨期连续性，做成同一条可重复、可检查、可移植的交付工作流。

| 产品范式 | 典型能力 | 本 skill 的处理方式 |
| --- | --- | --- |
| 聚合与个性化（Google News） | 按语言、地域、兴趣与历史活动选择内容 | 不宣传“个性化聚合”独占，用输入合同固定检索口径 |
| 多视角比较（Ground News） | 合并同一事件多来源，展示倾向、事实性与来源 | 不宣传“多视角合并”独占，用来源级别与证据状态显式标边界 |
| AI 摘要与问答（Particle） | 多种摘要视图、故事问答、时间线与来源链接 | 不宣传“AI 摘要”独占，保留条目后叠加编辑判断与验收 |
| 情报监测与去重（Feedly AI） | 主题/公司追踪、优先级排序、跨源去重 | 不宣传“监测/去重”独占，把弱证据条目转成下期具体检查任务 |

组合差异：

1. `为什么重要`：每条重点新闻的固定字段，明确影响对象、后果与当前相关性，而不只是复述事件
2. `本期点评`：跨条目的编辑综合，压缩成少量有区分度的周期信号
3. `认知延伸`：只从已保留、证据充分的条目生成，每条标注依据与 `性质：推断`
4. `继续跟踪`：弱证据事件、未决问题、实体、指标和时间范围被转成下一周期的具体检查任务
5. `证据边界`：正文条目带 `来源级别` 与 `证据状态`，待确认内容转入继续跟踪
6. `验收门`：检查单来源高影响条目、无依据因果、未查反证和字段缺失，阻断项阻止正式产物写出
7. `跨期连续性`：导出为用户可见的 JSON，由调用方、上期简报或自动化显式传递，不依赖隐藏记忆
8. `交付闭环`：`collect -> normalize/deduplicate -> rank/retain -> verify -> render -> cognition -> acceptance -> polish`
