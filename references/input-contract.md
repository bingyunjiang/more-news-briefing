...
12. `cognitive_features`: `compact`, `insight`, `analyst`, `all`, `off`, or a comma-separated subset of `interrogate`, `sprout`, `commentary`, and `continuity`; default `interrogate`
...
12. `cognitive_features`: `interrogate` (`compact`)

If the user asks to test, demo, preview, or see the cognitive layer, use `cognitive_features=all` unless they explicitly choose a smaller profile.
